import { createOciEmailSender, parseOciSmtpCredentials } from './oci.js';
...
export async function buildEmailSender(env: AppEnv): Promise<DocumentEmailSender | undefined> {
  ...
  if (env.MAIL_PROVIDER === 'oci') return createOciEmailSender(parseOciSmtpCredentials(env.OCI_SMTP_CREDENTIALS));
