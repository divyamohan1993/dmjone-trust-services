...
import { MAIL_FROM, messageText, messageHtml, recordsCc, type DocumentEmailSender } from './provider.js';
...
export function createOciEmailSender(credentials: OciSmtpCredentials, factory: TransportFactory = options => nodemailer.createTransport(options)): DocumentEmailSender {
  return {provider:'oci', prepare(message) {
    return JSON.stringify({from:MAIL_FROM,to:[message.to],cc:recordsCc(message.to),replyTo:'contact@dmj.one',
    ...
    try {
      ...
      const expected=[...(message.to as string[]),...((message.cc??[]) as string[])].map(v=>v.toLowerCase());
      const accepted=new Set((info.accepted??[]).map(v=>v.toLowerCase()));
      return expected.length>0 && expected.every(v=>accepted.has(v)) && !(info.rejected?.length) && typeof info.messageId === 'string'
